...
#include <hal/link/desktop/abstract-link.h>
...
private:
  ...
  std::vector<float> _ranges{0};
  std::vector<float> _intensities{0};
  ...
  //std::shared_ptr<PingPort> m_link = AbstractLink::openUrl("serial:/dev/ttyUSB0:115200");
  //Ping360 _sensor = Ping360(*m_link);

  //std::shared_ptr<AbstractLink> port = AbstractLink::openUrl("serial:/dev/ttyUSB0:115200");
  //Ping360 _sensor = Ping360(*port.get());
